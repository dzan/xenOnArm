    ...
    for ( ; ; )
    {
        ...
        do {
            ...
            for ( j = order; j <= MAX_ORDER; j++ )
                if ( (pg = page_list_remove_head(&heap(node, zone, j))) && (pg->count_info == PGC_state_free) )
                    goto found;
...
struct page_info *alloc_domheap_pages_11(
    struct domain *d, unsigned int order, unsigned int memflags, paddr_t start)
{
    struct page_info *pg = NULL;

    ASSERT(!in_irq());

    pg = mfn_to_page(start >> PAGE_SHIFT);
        if (unlikely(pg->count_info != PGC_state_free)) {
            /* printk("DEBUG %s %d, BUG addr %#010llx\n", __func__, __LINE__, start); */
            /* BUG(); */
        }
        pg->count_info = PGC_state_inuse | PGC_allocated | 1;
        pg->u.inuse.type_info = PGT_validated | PGT_writable_page;
...
void free_domheap_pages(struct page_info *pg, unsigned int order)
{
    ...
    if (0x40000 <= page_to_mfn(pg) && page_to_mfn(pg) < 0x60000) {
        return;
